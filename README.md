...
* **Delimiter Separation**: To isolate the steganographic payload, a unique signature string (`####PROJECT_HERMES_EOF####`) is written between the video file EOF and the ciphertext payload. Media players ignore data following their expected container format EOF, keeping playback functional.
...
```
Project-Hermes/ (Repository Root & Project Directory)
├── OOPS Capstone Project Presentation.pptx
├── README.md                          # System Documentation
├── Project-Hermes.jar                 # Compiled Executable JAR Package
├── OOPS_Cryptography-Steganography_Project.iml
├── out/                               # Build Outputs
└── src/
    └── main/
...
### Execution
Compile and run the verification engine from the repository root:
```powershell
